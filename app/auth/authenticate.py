    ...
    def post(self):
        ...
        if all(key in reg_info.keys() for key in search_keys):
            ...
            if email_validation == False:
                return jsonify({"message": "you have entered a email , Please try again"}), 400

            validate_username = validate.validate_characters(user_name)
            if not validate_username:
                return jsonify({"message": "you have entered invalid username format, Please try again"}), 400
